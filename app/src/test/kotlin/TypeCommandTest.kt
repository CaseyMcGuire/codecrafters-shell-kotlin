...
class TypeCommandTest {
  ...
  fun `identifies a known builtin via resolveBuiltin`() {
    ...
    assertEquals("echo is a shell builtin", type.runCaptured("type", listOf("echo")).stdout)
  }
  ...
  fun `identifies every registered builtin`() {
    ...
    shell.builtins.forEach { builtin ->
      assertEquals(
        ...
        type.runCaptured("type", listOf(builtin.text)).stdout,
      )
    }
  }
  ...
  fun `reports not found when neither builtin nor PATH match`() {
    assertEquals("nope: not found", typeWith().runCaptured("type", listOf("nope")).stdout)
  }

  @Test
  fun `with no args returns empty result`() {
    val result = typeWith().runCaptured("type", emptyList())
  ...
  fun `only inspects the first arg`() {
    val type = typeWith(resolveBuiltin = { name -> if (name == "echo") EchoCommand() else null })
    assertEquals("echo is a shell builtin", type.runCaptured("type", listOf("echo", "exit")).stdout)
  }
